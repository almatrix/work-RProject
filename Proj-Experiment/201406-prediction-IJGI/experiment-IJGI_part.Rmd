...
# global matrix
pr.t.mat = get.temporal.impact(checkin.in.poly)
pr.w.mat = get.weighted.meteorological.impact(fit)
# pr.w.mat = get.meteorological.impact2(checkin.in.poly)
pr.s.mat = get.spatial.impact(checkin.in.poly)
pr.d = unlist(get.denominator(checkin.in.poly))

checkin.in.poly$flag = paste(checkin.in.poly$hour,checkin.in.poly$POSTAL,
                             checkin.in.poly$conds)
flag.list = split(checkin.in.poly, checkin.in.poly$flag)
counter = 0 
prediction = do.call(rbind, lapply(flag.list,function(i){
    counter <<- nrow(i)+counter
    print(paste(Sys.time(),"processed",counter,"records."))
    ...
    pr.ave = pr.t * pr.s * pr.w / pr.d ^ 2 
    
    # order the probability vector (get first 30)
    pred.d = sort(pr.d, decreasing = TRUE)
    pred.t = sort(pr.t, decreasing = TRUE)
    pred.s = sort(pr.s, decreasing = TRUE)
    pred.w = sort(pr.w, decreasing = TRUE)
    pred.all = sort(pr.ave, decreasing = TRUE)
    ...
    df = do.call(rbind, lapply(real.list,function(real){
        ...
        df = data.frame("None"=p1,"Temporal"=p2,"Spatial"=p3,
                        "Meteorological"=p4,"All"=p5,
                        "Hour"=hour,"Weather"=conds,"Postal"=polygon,
                        "Category"=real)
...
save(prediction,file="../../global/data/Rda-saved/prediction.records.weather1.Rda")
print(Sys.time())
...
# density
ggplot(melt(prediction[,1:5]))+
    stat_density(aes(x=value,group=variable,color=variable),
                 size=0.5,fill="grey",alpha=0.1,position="identity")+
    xlim(0,50)+
    xlab("Length to be correct")+
    ylab("Density")
...
rate = do.call( rbind, lapply(1:30, function(N){ # list length: N
    # number of correct
    c1 = length(which(prediction$None<=N)) / nrow(prediction)
    c2 = length(which(prediction$Temporal<=N)) / nrow(prediction)
    c3 = length(which(prediction$Spatial<=N)) / nrow(prediction)
    c4 = length(which(prediction$Meteorological<=N)) / nrow(prediction)
    c5 = length(which(prediction$All<=N)) / nrow(prediction)
    
    data.frame("Length"=N,"None"=c1,"Temporal"=c2,"Spatial"=c3,
               "Meteorological"=c4,"All"=c5)
}) )

ggplot(melt(rate,id="Length"),aes(x=Length,y=value,color=variable))+
    geom_line()+
...
performance.vs.hour = do.call(rbind,lapply(split(prediction,prediction$Hour),
                                           function(i){
    rbind(data.frame("Precision"=length(which(i$All<=5)) / nrow(i),
               "Hour"=i[1,"Hour"],
               "Massive"=nrow(i),
               "Length"="5"),
          data.frame("Precision"=length(which(i$All<=10)) / nrow(i),
               "Hour"=i[1,"Hour"],
               "Massive"=nrow(i),
               "Length"="10"))
}))
ggplot(performance.vs.hour,aes(x=Massive,y=Precision,color=Length))+
    geom_point()+
    geom_text(aes(label=Hour),hjust=0, vjust=0)+
    ggtitle("Precision @ N=5 V.S. Time")


performance.vs.space = do.call(rbind,lapply(split(prediction,prediction$Postal),
                                            function(i){
    rbind(data.frame("Precision"=length(which(i$All<=5)) / nrow(i),
               "Postal"=i[1,"Postal"],
               "Massive"=nrow(i),
...
ggplot(performance.vs.space,aes(x=Massive,y=Precision,color=Length))+
    geom_point()+
    ggtitle("Precision @ N=5 V.S. Space")


performance.vs.conds = do.call(rbind,lapply(split(prediction,prediction$Weather),
                                            function(i){
    rbind(data.frame("Precision"=length(which(i$All<=5)) / nrow(i),
               "Weather"=i[1,"Weather"],
               "Massive"=nrow(i),
...
ggplot(performance.vs.conds,aes(x=Massive,y=Precision,color=Length))+
    geom_point()+
    geom_text(aes(label=Weather),hjust=0, vjust=0)+
    ggtitle("Precision @ N=5 V.S. Weather")



performance.vs.user = do.call(rbind, lapply(split(prediction,prediction$user),
                                            function(i){
    rbind(data.frame("Precision"=length(which(i$All<=5)) / nrow(i),
               "User"=i[1,"user"],
               "Massive"=nrow(i),
...
performance.vs.cate = do.call(rbind,lapply(split(prediction,prediction$Category),
                                           function(i){
    rbind(data.frame("Precision"=length(which(i$All<=5)) / nrow(i),
