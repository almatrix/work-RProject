...
get.meteorological.impact2 <- function(dataframe){
    ...
    list.pr = lapply(split(dataframe,dataframe$conds), function(conds){
#         data.in.conds = dataframe[dataframe$conds==conds,]
...
counter = 0; prediction = data.frame()
temp = lapply(flag.list,function(i){
    ...
    prediction <<- rbind(prediction, df)
    
    NA
})

rm(temp)
save(prediction,file="../../global/data/Rda-saved/prediction.records.weather2.Rda")
...
ggplot(melt(prediction[,1:5]))+
    stat_density(aes(x=value,group=variable,color=variable),
                 size=1.2,fill="grey",alpha=0.4,position="identity")+
    xlim(1,50)


# precision and recall
rate = lapply(1:30, function(N){ # list length: N
    # number of correct
...
performance.vs.hour = lapply(split(prediction,prediction$Hour),function(i){
    data.frame("Precision"=length(which(prediction$All<=5)) / nrow(i),
               "Hour"=i[1,"Hour"],
               "Massive"=nrow(i))
})
...
performance.vs.space = lapply(split(prediction,prediction$Postal),function(i){
    data.frame("Precision"=length(which(prediction$All<=5)) / nrow(i),
               "Postal"=i[1,"Postal"],
               "Massive"=nrow(i))
})
...
performance.vs.conds = lapply(split(prediction,prediction$Weather),function(i){
    data.frame("Precision"=length(which(prediction$All<=5)) / nrow(i),
               "Weather"=i[1,"conds"],
               "Massive"=nrow(i))
})
...
performance.vs.user = lapply(split(prediction,prediction$user),function(i){
    rbind(data.frame("Precision"=length(which(prediction$All<=5)) / nrow(i),
               "User"=i[1,"user"],
               "Massive"=nrow(i),
               "Length"="5"),
          data.frame("Precision"=length(which(prediction$All<=10)) / nrow(i),
               "User"=i[1,"user"],
               "Massive"=nrow(i),
               "Length"="10"))
})
...
performance.vs.cate = lapply(split(prediction,prediction$Category),function(i){
    rbind(data.frame("Precision"=length(which(prediction$All<=5)) / nrow(i),
               "Category"=i[1,"cate"],
               "Massive"=nrow(i),
               "Length"="5"),
