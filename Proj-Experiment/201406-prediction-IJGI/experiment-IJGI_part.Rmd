...
pr.t = get.temporal.impact(checkin.NY)
```


* relevance contextualized by spatial factor
```{r}
cate.distr = cate.distr.in.poly(checkin.NY,poly.attr="POSTAL",
                                    cate.attr="cate_l2")
get.spatial.impact <- function(SPDF, lat, lon,poly.attr="POSTAL"){
    spt = SpatialPointsDataFrame(coords=data.frame(x=lon,y=lat),
                                  data=data.frame(lon=lon,lat=lat), 
                                  proj4string=CRS(crs.wgs84.str))
    
    # intersect the two layers, and copy the information specified by 
    # "copy.attr" from the intersecting polygon to the point
    region.id = over(spt, SPDF)[,poly.attr]
    cate.distr[region.id, ]
}
```
...
apply(checkin.NY[1:10,],1,function(i){
    hour = i$hour
    conds = i$conds
    lat = i$lat
    lon = i$lon
    ...
    pr.s = get.spatial.impact(SPDF,lat=lat,lon=lon)
    wgt.w = get.weather.weight(fit)
    pr.w = get.weighted.meteorological.impact(checkin.NY,conds)
    pr.d = get.denominator(checkin.NY)
    
    pr.ave = pr.t * pr.s * pr.w / pr.d
    
    pred = sort(pr.ave, decreasing = TRUE)[1:20]
