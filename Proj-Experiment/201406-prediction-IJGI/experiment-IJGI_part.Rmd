...
```{r}
checkin.in.poly = point.in.poly(checkin, SPDF, copy.attr=poly.attr)
...
result = lapply(1:nrow(checkin.in.poly),function(i){
    if(i%%500==1)print(paste(Sys.time(),i))
    
    checkin = checkin.in.poly[i,]
    hour = checkin$hour
    conds = checkin$conds
    polygon = checkin$POSTAL
    real = as.character(checkin$cate_l2)
    ...
    res = rep(0,100)
    for (p in 1:20){
        if (names(pred.d[p])==real){res[p:20]=1}
        if (names(pred.t[p])==real){res[(20+p):40]=1}
        if (names(pred.s[p])==real){res[(40+p):60]=1}
        if (names(pred.w[p])==real){res[(60+p):80]=1}
        if (names(pred.all[p])==real){res[(80+p):100]=1}
    }
...
```{r}
test = apply(result,2,function(i){
    sum(i) / nrow(checkin.in.poly)
})
