...
have a look at the intuitive visualizaiton...
...
What do the statistics say? We found differnt clusters under differnt 
categories, each clustering indicating the "hotspots" for that specific 
category.
...
## SpatialPointDataFrame from checkin dataset 
checkin.spt = SpatialPointsDataFrame(coords=checkin.NY[,c("lon","lat")],
                                     data=checkin.NY[,c("gid","cate_l1")], 
                                     proj4string=CRS("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0"))
# overlay the point data with zipcode polygon
overlap = over(x=checkin.spt,
                       y=readOGR(dsn="../../global/data/shapefiles",
                                 layer="NYC_zipcode"))
checkin.NY$zip = overlap$POSTAL

# plot the dominant category of each zipcode area
zip.category = lapply(split(checkin.NY,checkin.NY$zip),
                        function(i){
    # the corresponding points in that zipcode
    category = names(which.max(table(i$cate_l1)))
    zip = i[1,"zip"]
    data.frame("zip"=zip,"cate_l1"=category)
})
zip.category<- do.call(rbind,zip.category)
shape = readOGR(dsn="../../global/data/shapefiles",layer="NYC_zipcode")
shape@data$id = rownames(shape@data)
shape@data = merge(shape@data,zip.category,by.x="POSTAL",by.y="zip",all.x=TRUE) 
shape.points = fortify(shape, region="id")
shape.df = join(shape.points, shape@data, by="id")

png("test.png",height=6*ppi, width=8*ppi,res=ppi)
ggplot() + 
    geom_polygon(data=shape.df,aes(long,lat,group=group,fill=cate_l1))+
    coord_map()+
