...
## spatial clustering regardless of categories
clusters = spatial.clustering(checkin.NY,wss.control = 0.01)
venues = clusters[["point.unique"]]
centers = clusters[["centers"]]
venues.withcategory = merge(x=unique(checkin.NY[,c("cate_l1","lat","lon")]), y=venues, 
      by.x=c("lat","lon"), by.y =c("lat.x","lon.x"), all.x=TRUE)

centers.withcategory = 
    lapply(split(venues.withcategory, venues.withcategory$cid.ordered),function(i){
        center = centers[which(centers$cid.ordered == i[1,"cid.ordered"]),] 
        # the most frequently occuring value
        center$cate_l1 = as.factor(names(which.max(table(i$cate_l1 ))))
        center
    })

centers.withcategory = do.call(rbind,centers.withcategory)
png("test.png",height=6*ppi, width=6*ppi,res=ppi)
 point.plot(centers.withcategory, x="lon.center",y="lat.center",
                         color= "cate_l1", color.concrete=FALSE,
                         point.size = 2, alpha = 0.7,
                         basemap=basemap)
dev.off()
