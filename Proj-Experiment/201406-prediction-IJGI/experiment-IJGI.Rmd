...
freq.plot(checkin.NY,plot.title="New York City")
```
...
```{r, fig.width=8, fig.height=6, dpi=300}
time.distribution.plot(checkin.NY,plot.title="New York City")
```
...
```{r}
saveGIF({
  point.animation.plot(checkin.NY,plot.title="New York City",
                       axis.size=18,title.size=24,legend.size=18,plot.size=3)
...
plots <- lapply(split(checkin.NY,checkin.NY$cate_l1),function(ci){
    ...
    gg.map <- ggplot() +
        geom_point(data=points,
                   aes(x=lon.x,y=lat.x),size=1, alpha=0.05,color="#55B1F7") +
        geom_point(data=centers,
                   aes(x=lon.center,y=lat.center,color=cid.ordered),
                   size=log(centers$size,5),
                   alpha = 0.7)+
        ...
        geom_text(aes(x = -74.13, y = 40.87), label = lbl, size=3) +
        ggtitle(ci[1,"cate_l1"]) +
        coord_map()
    
    shape = readOGR(dsn="../../global/data/shapefiles", 
...
png("test.png",width=15*ppi,height=6*ppi,res=ppi)
