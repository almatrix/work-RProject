...
```{r spatial,echo=TRUE,eval=TRUE,fig.width=10,fig.height=11}
# frequency statistics
...
categorical.scaler.zip <- xtabs2(data = checkin.poly.list[[1]],
...
# apply
checkin.continuous = merge(checkin.poly.list[[1]][,c(2,9,10,12,25)],
...
+ making the computation more timely efficient;
+ equally (even slightly better) explain the data; (~25%)
+ provide much more significant regression coefficicent;
...
categorical.scaler.hour <- xtabs2(data = checkin.poly.list[[1]],
...
And the conclusions for spatial context hold true also for temporal one. More interestingly, the model after transformation provides better describtion of the data: from merely 8% to 38%. 

Then how about if we consider both?

```{r st.model,echo=TRUE,eval=TRUE}
# adding time+space information
t0=Sys.time()
glmodel5 <- glm(cate_l2 ~  prob.est.zip + hour, 
               data=checkin.continuous, family=binomial(link="logit"))
Sys.time()-t0
summary(glmodel5)
...
# considering interactions between time and space
t0=Sys.time()
glmodel7 <- glm(cate_l2 ~  prob.est.hour * prob.est.zip , 
               data=checkin.continuous, family=binomial(link="logit"))
Sys.time()-t0
summary(glmodel7)
...
The last two models show that after the transformation, the interactions can be neglected.