...
data.space.coor$outlier<-with(data.space.coor,
                             ifelse(space %in% c("60629","60666","60609","60612","90045","11371","11430","11368","10452"),
                                    as.character(space),""))
eqs.prediction = data.frame()
...
#         eqs.pred$record = with(eqs.pred,atan(log(x/max(x))))
        }
    else if(i[1,"variable"]=="delta"){ 
        model = lm(i,formula=value~atan(log(record/100)))
        eqs.pred=data.frame(record = c(min(i$record):max(i$record)))
#         eqs.pred$record = with(eqs.pred,atan(log(x/100)))
        }
    else{
        model = lm(i,formula=value~atan(log(record*100)))
        eqs.pred=data.frame(record = c(min(i$record):max(i$record)))
#         eqs.pred$record = with(eqs.pred,atan(log(x*10)))
        }
#     model =lm(i,formula=value~log(record))
    ...
    eqs.prediction <<- rbind(eqs.prediction, eqs.pred)
...
pdf("plots/fig8.coor.space.pdf",width=7,height=3.5,colormodel = "grey")
theme_new<-theme_set(theme_bw(base_size = 10))
theme_new<-theme_update(legend.position="none")
ggplot(data.space.coor,aes(x=record,y=value))+#geom_point(size=3,shape=21,fill=NA)+
    facet_grid(variable~city,scales="free",
               labeller=labeller(variable=label_bquote(Pi["ST"]^.(x))))+
    geom_line(data=eqs.prediction,aes(x=record,y=y),color="grey")+
#     geom_smooth(method=lm,formula=y~atan(x-2),se=F)+
    geom_point(size=1,aes(shape=(outlier!="")))+
    geom_text(aes(label=outlier),vjust=1.5,size=2.5)+
    xlab("Activeness (Hourly records)") +
    ylab(expression(Pi["ST"])) +
#     ggtitle(expression(paste("Temporal Correlation (",Pi["ST"]," and Activeness)")))+
    scale_x_log10()+
...
pdf("plots/fig9.coor.space2.pdf",width=7,height=4.5,colormodel = "grey")
theme_new<-theme_set(theme_bw(base_size = 10))
theme_new<-theme_update(legend.position="none",
          axis.text.x=element_text(angle=50,vjust=1,hjust=1))
ggplot(space.stat[which(space.stat$space %in% c("60629","60666","60609","60612","90045","11371","11430","11368","10452")),])+
