...
tiff("plots/checkin_points_cities.png", width = 10*ppi, height = 3.5*ppi, res=ppi)
...
#                 formula=y~x,method=glm,family=gaussian(link='log'))+
    ...
    labs(x="Personal Checkin Counts [x]",y="CDF [Pr(X<=x)]")+
    facet_wrap(~city)+
    theme(strip.text=element_text(size=8),axis.text = element_text(size=8),
              axis.title = element_text(size=8),plot.title = element_text(size=10),
          plot.margin=unit(c(.05,.05,.05,.05),"npc"))
...
lapply(1:3,function(city.index){
    ...
    #         geom_vline(xintercept=xintercepts[idx])+
            annotate("text",x=200,y=1.6,size=2.5,
...
tiff(paste("plots/user.clustering_.tiff"),width=8*ppi,height=4.5*ppi,res=ppi)
...
tiff("plots/pi.max.min.density.tiff",width=8*ppi,height=3.5*ppi,res=ppi)
...
data.pi.density$variable<-factor(data.pi.density$variable,labels=c("max","min"))
tiff("plots/pi.records.tiff",width=8*ppi,height=3.5*ppi,res=ppi)
ggplot(data.pi.density,aes(x=Records,y=value))+
    facet_grid(Condition~City+variable,labeller=labeller(variable=label_bquote(Pi^.(x))))+
    geom_point(shape=21,fill="white",size=1,alpha=0.6)+scale_x_log10()+
...
gga<-ggplot(data,aes(x=hour,y=pi.max) )+
    facet_wrap(~city)+
    geom_point()+
    geom_smooth(aes(group = 1),se=F, method = "lm", formula = y ~ poly(x, 12),size=0.8) +
    xlab("Hour of Day\n(a)") +
    ylab(expression(Pi^max)) +ggtitle(expression(paste("Temporal Distribution of ",Pi^"max")))+
    ...
    theme(legend.position="bottom",strip.text=element_text(size=7),axis.text = element_text(size=6),
          ...
          plot.margin=unit(c(.05,.05,.05,.05),"npc"))
ggb<-ggplot(data,aes(x=hour,y=pi.min) )+
    facet_wrap(~city)+
    geom_point()+
    geom_smooth(aes(group = 1),se=F, method = "lm", formula = y ~ poly(x, 12),size=0.8) +
    xlab("Hour of Day\n(b)") +
    ylab(expression(Pi^min)) +ggtitle(expression(paste("Temporal Distribution of ",Pi^"min")))+
    scale_x_discrete(breaks=c("00","04","08","12","16","20"))+
    theme(legend.position="bottom",strip.text=element_text(size=7),axis.text = element_text(size=6),
          axis.title = element_text(size=7),plot.title = element_text(size=8),
          plot.margin=unit(c(.05,.05,.05,.05),"npc"))
ggc<-ggplot(data,aes(x=hour,y=record))+
    facet_wrap(~city)+geom_point()+
    geom_smooth(aes(group = 1),se=F, method = "lm", formula = y ~ poly(x, 12),size=0.8)+
    xlab("Hour of Day\n(c)") +
    ylab("Check-ins") +ggtitle("Temporal Distribution of Activeness")+
    scale_x_discrete(breaks=c("00","04","08","12","16","20"))+
    scale_y_log10()+
#     scale_y_continuous(labels=percent)+
    theme(strip.text=element_text(size=7),axis.text = element_text(size=6),
              axis.title = element_text(size=7),plot.title = element_text(size=8),
          plot.margin=unit(c(.05,.05,.05,.05),"npc"))
eqs1 <- ddply(data,.(city),function(i){
    model = lm(i,formula=pi.max~record) 
...
eqs2 <- ddply(data,.(city),function(i){
...
ggd<-ggplot(data,aes(x=record,y=pi.max))+#geom_point(size=3,shape=21,fill=NA)+
    facet_grid(~city,scales="free_x")+
    geom_text(aes(label=hour),size=3)+
    geom_smooth(method=lm,formula=y~exp(x))+
    xlab("Check-ins\n(a)") +
    ylab(expression(Pi^max)) +
    ggtitle(expression(paste("Temporal Correlation (",Pi^"max"," and Activeness)")))+
    scale_x_log10()+
#     scale_x_continuous(labels=percent)+
    geom_text(data=eqs1,aes(x=500,y=0.73,label=V1),inherit.aes=FALSE,
...
gge<-ggplot(data,aes(x=record,y=pi.min))+#geom_point(size=3,shape=21,fill=NA)+
    facet_grid(~city,scales="free_x")+
    geom_smooth(method=lm,formula=y~x)+
    geom_text(aes(label=hour),size=3)+
    xlab("Check-ins\n(b)") +
    ylab(expression(Pi^min)) +
    ggtitle(expression(paste("Temporal Correlation (",Pi^"min"," and Activeness)")))+
#     scale_x_continuous(labels=percent)+
    scale_x_log10()+
    geom_text(data=eqs2,aes(x=500,y=0.2,label=V1),inherit.aes=FALSE,
                  parse = TRUE,hjust=0,size=3)+
    theme(axis.text = element_text(size=8),legend.position="none",
          plot.margin=unit(c(.05,.05,.05,.05),"npc"),
              axis.title = element_text(size=10),plot.title = element_text(size=10))

# tiff("plots/pi.min.hourly.tiff",width=8*ppi,height=5*ppi,res=ppi)
# grid.arrange(
#                 arrangeGrob(gga,ggb,ggc,nrow=3,heights=c(1,1,1)),
#                 arrangeGrob(ggd,gge,nrow=2),
#                 ncol=2,widths=c(1,1.2))
tiff("plots/pi.time.tiff",width=8*ppi,height=5*ppi,res=ppi)
grid.arrange(ggd,gge,ncol=1,nrow=2)
dev.off()
...
gga2<-ggplot(data3,aes(x=record,y=pi.max))+facet_grid(~city,scales="free_y")+
    ...
    geom_text(aes(label=disp.name.max),size=3,vjust=1.1,hjust=0.5)+
    geom_smooth(method=glm,formula=y~exp(x))+
    xlab("Check-ins\n(a)") +
    geom_text(data=eqs3,aes(x=10,y=0.67,label=V1),inherit.aes=FALSE,
                  parse = TRUE,hjust=0,size=3)+
    ylab(expression(Pi^max)) +
...
ggb2<-ggplot(data3,aes(x=record,y=pi.min))+facet_grid(~city,scales="free_y")+
    ...
    geom_text(aes(label=disp.name.min),size=3,vjust=1.1,hjust=0.5)+
    geom_smooth(method=glm,formula=y~x,family=gaussian(link='log'))+
    xlab("Check-ins\n(b)") +
    geom_text(data=eqs4,aes(x=10,y=0.1,label=V1),inherit.aes=FALSE,
                  parse = TRUE,hjust=0,size=3)+
    ylab(expression(Pi^min)) +
    ggtitle(expression(paste("Spatial Correlation (",Pi^"min"," and Activeness)")))+
    scale_x_log10()+scale_y_continuous(limits=c(0.1,1))+
    scale_shape_manual(name="Outlier",values=c(16,2))+
    theme(axis.text = element_text(size=8),legend.position="none",
          plot.margin=unit(c(.05,.05,.05,.05),"npc"),
              axis.title = element_text(size=10),plot.title = element_text(size=10))
tiff("plots/pi.spatial.1.tiff",width=8*ppi,height=5*ppi,res=ppi)
# grid.arrange(arrangeGrob(gga2,ggb2,ncol=2),
#              gg.outliers,
...
gg.outliers = ggplot(outlier.stat)+facet_wrap(city~space)+
    geom_histogram(aes(x=category,y=Freq),stat="identity")+
    coord_flip()+
...
tiff("plots/pi.spatial.2.tiff",width=8*ppi,height=4*ppi,res=ppi)
gg.outliers 
dev.off()
```
...
gg.maps<-lapply(1:3,function(city.index){
    ...
    map1<-map.plot(mapdf=shape.df,more.aes=aes(fill=resid.max.T),color="grey",size=0.3)+
#         ggtitle(city.guide$city[city.index])+
        ggtitle(bquote(atop(.(city.guide$city[city.index])~": Outliers of"~Pi^"max" )))+
        labs(x="Longitude",y="Latitude")+
        scale_fill_manual(values=c("white","grey"),name="Outliers")+
        geom_text(data=shape.df,aes(x=lon.cent,y=lat.cent,label=disp.name.max),size=3)+
        theme(axis.text = element_text(size=8),legend.position="none",
              legend.title=element_text(size=8),legend.text=element_text(size=8),
              axis.title = element_text(size=10),plot.title = element_text(size=10),
          plot.margin=unit(c(.05,.05,.05,.05),"npc"))
    map2<-map.plot(mapdf=shape.df,more.aes=aes(fill=resid.min.T),color="grey",size=0.3)+
#         ggtitle(city.guide$city[city.index])+
        ggtitle(bquote(atop(.(city.guide$city[city.index])~": Outliers of"~Pi^"min" )))+
        labs(x="Longitude",y="Latitude")+
        scale_fill_manual(values=c("white","grey"),name="Outliers")+
        geom_text(data=shape.df,aes(x=lon.cent,y=lat.cent,label=disp.name.min),size=3)+
        theme(axis.text = element_text(size=8),legend.position="none",
              legend.title=element_text(size=8),legend.text=element_text(size=8),
              axis.title = element_text(size=10),plot.title = element_text(size=10),
          plot.margin=unit(c(.05,.05,.05,.05),"npc"))
...
tiff("plots/outliers.map.tiff",width=10*ppi,height=7.5*ppi,res=ppi)
grid.arrange(gg.maps[[1]][[1]],gg.maps[[2]][[1]],gg.maps[[3]][[1]],
             gg.maps[[1]][[2]],gg.maps[[2]][[2]],gg.maps[[3]][[2]],
             ncol=3,nrow=2,widths=c(1.05,1,1.1))
dev.off()
```