...
#                 formula=y~x,method=lm)+
    ...
    xlab("Personal records [x]")+ylab("PMF [Pr(X=x)]")+
...
#                 formula=y~x,method=glm,family=gaussian(link='log'))+
    ...
    labs(x="Personal records [x]",y="CDF [Pr(X<=x)]")+
    facet_wrap(~city)+theme_bw(base_size = 10)
#     theme(strip.text=element_text(size=8),axis.text = element_text(size=8),
#               axis.title = element_text(size=8),plot.title = element_text(size=10),
#           plot.margin=unit(c(.05,.05,.05,.05),"npc"))
...
ggplot(data.individual.coor,
    ...
    labs(x="Activeness (Individual records)",y=expression(Pi["ST"]))+theme_bw(base_size = 10)
...
#     geom_smooth(method=loess,formula=y~x)+
    ...
    xlab("Activeness (Hourly records)") +
...
pdf("plots/fig7.coor.time2.pdf",width=7,height=4.5,colormodel = "grey")
theme_new<-theme_set(theme_bw(base_size = 10))
...
data.space.coor$variable<-factor(data.space.coor$variable,
                                labels=c("max","min",expression(delta)))
data.space.coor$trans.x<-with(data.space.coor,
                              ifelse(variable=="max",10,3))
data.space.coor$outlier<-with(data.space.coor,
#                               ifelse(space %in% c("60827","60633","60609","60612","60629","60666",
#                                                   "90045","90028","90036","91302","91330","90067",
#                                                   "11371","10452","11430","11368","11109","11001"),
                              ifelse(space %in% c("60629","60666","90045","11371","11430"),
                                     as.character(space),""))

eqs.space <- dlply(data.space.coor,.(city,variable),function(i){
    if(i[1,"variable"]=="max"){ 
        model = lm(i,formula=value~atan(log(record/max(record))))
        x = i$record
        x.trans = atan(log(x/max(x)))
        }
    else if(i[1,"variable"]=="delta"){ 
        model = lm(i,formula=value~atan(log(record/100)))
        x = i$record
        x.trans = atan(log(x/100))
        }
    else{
        model = lm(i,formula=value~atan(log(record*10)))
        x=i$record
        x.trans = atan(log(x*10))
        }
#     model =lm(i,formula=value~log(record))
    eq <- substitute(atop(italic(r)^2~"="~r2*","~~italic(p)~"="~pvalue),
                     list(r2 = format(summary(model)$r.squared, digits = 3),
    ...
    y = predict(model, newdata = x.trans)
eq
    #list(eq, data.frame(x=x,y=y))
})
eqs.space$pos.x=rep(c(10,3,10),each=3)
eqs.space$pos.y=rep(c(0.7,0.1,2.7),3)
eqs.smooth <-lapply(split(data.space.coor,data.space.coor$variable),function(i){
    if(i[1,"variable"]=="max"){ 
        model = lm(i,formula=value~atan(log(record/max(record))))
        smooth.data <- data.frame(record= atan(log(c(1:max(i$record))/max(i$record))) ) 
        }
    else if(i[1,"variable"]=="delta"){ 
        model = lm(i,formula=value~atan(log(record/100)))
        smooth.data <- data.frame(record=atan(log(c(1:max(i$record))/100)))  
        }
    else{
        model = lm(i,formula=value~atan(log(record*10)))
        smooth.data <- data.frame(record=atan(log(c(1:max(i$record))*10)))  
        }
    smooth.data$y <- predict(model, newdata = smooth.data) 
    smooth.data
})
# eqs2 <- ddply(data.hour.coor,.(city,variable),function(i){
#     model = lm(i,formula=value~record) 
...
# })
# 
pdf("plots/fig8.coor.space3.pdf",width=7,height=3.5,colormodel = "grey")
ggplot(data.space.coor,aes(x=record,y=value))+#geom_point(size=3,shape=21,fill=NA)+
    facet_grid(variable~city,scales="free",
               labeller=labeller(variable=label_bquote(Pi["ST"]^.(x))))+
    geom_smooth(method=lm,formula=y~atan(x-2),se=F)+
    geom_point(size=1)+
    geom_text(aes(label=space),vjust=1.5,size=1)+
    xlab("Activeness (Hourly records)") +
    ylab(expression(Pi["ST"])) +
#     ggtitle(expression(paste("Temporal Correlation (",Pi["ST"]," and Activeness)")))+
    scale_x_log10()+
