...
#     sample.index = sample(1:nrow(point.data),floor(nrow(point.data)*0.2))
    point.plot(point.data,
#                alpha=0.3,size=1,fill=NA,color="#333333",
               shape=21,color="white",fill="#111111",size=1,alpha=0.2,
...
dev.off()
#####
tiff("plots/test.tiff",width=4*ppi,height=4*ppi,res=ppi)
gg.maps[[1]]
dev.off()
```
...
# cairo_pdf("plots/fig3.user.agg.pdf",width=8,height=4.5,antialias = "gray")
# svg("plots/fig3.user.agg.svg",width=8,height=4.5,antialias = "gray")
postscript("plots/fig3.user.agg.ps",width=8,height=4.5,bg="transparent",colormodel="gray",horizontal = F)
...
#     geom_smooth(method=loess,formula=y~x)+
    ...
    geom_point(data=data.hour.coor[!(data.hour.coor$hour %in% c("01","05","19")),],size=5,shape=21,fill="#999999",color="#eeeeee")+
    geom_text(data=data.hour.coor[!(data.hour.coor$hour %in% c("01","05","19")),],aes(label=hour),size=2,color="#efefef")+
    geom_point(data=data.hour.coor[which(data.hour.coor$hour %in% c("01","05","19")),],size=6,shape=21,fill="#666666",color="#eeeeee")+
    geom_text(data=data.hour.coor[which(data.hour.coor$hour %in% c("01","05","19")),],aes(label=hour),size=2.5,color="white")+
...
hour.stat$log.freq=with(hour.stat,ifelse(Freq==0,0,log(Freq)))
...
#     geom_rect(data=data.space.coor[which(data.space.coor$type%in%c("A","B")),],
#         aes(xmin=exp(log(record)-0.23),xmax=exp(log(record)+0.23),ymin=value-0.1*(max(value)-min(value)),ymax=value),fill="#eeeeee")+
