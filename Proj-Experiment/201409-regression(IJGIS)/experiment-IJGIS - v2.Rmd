...
entropies.etc.list = lapply(1:3,function(city.index){
    ...
    entropies.etc=lapply(user.list,function(i){
        ...
        unc = get.unc.entropy.etc(i)
        time = get.conditional.entropy.etc(i,"hour","Hour")
        space = get.conditional.entropy.etc(i,spatial.attr[city.index],"Space")
        
        i$ST = paste(i$hour, i[,spatial.attr[city.index]])
        st = get.conditional.entropy.etc(i,"ST","ST")
        ...
        time.detail$city = city.names[city.index]
        space.detail=unique(space$detail[,-c(2,3,6)])
        space.detail$city = city.names[city.index]
        st.detail=unique(st$detail[,-c(2,3,6)])
        st.detail$city = city.names[city.index]
...
ggplot(melt(data=entropies.etc.df[,c(1,3,4,10)],id.vars=c("Condition","City")))+
