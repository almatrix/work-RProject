...
get.unc.entropy.etc.v2 = function(data,data.global){
    data$venue_id=factor(data$venue_id) # do not statistic none-used venue
    venue.freq = as.data.frame(table(data$venue_id))
    ...
    prior.freq = as.data.frame(table(data.global$venue_id))
    colnames(prior.freq) = c("venue_id","Freq")
    prior.freq$prob = prior.freq$Freq / sum(prior.freq$Freq)
    prior.freq = subset(prior.freq,as.character(venue_id) %in% as.character(venue.freq$venue_id))
    pi.min = get.pi.min.from.freq(venue.freq, prior.freq$prob)
#     pi.min = max(venue.freq$Freq) / sum(venue.freq$Freq)
...
get.conditional.entropy.etc.v2 = function(data,condition,condition.name,data.global){
    data$venue_id=factor(data$venue_id) # do not statistic none-used venue
    
    freq.joint = as.data.frame(xtabs(data=data,
                               as.formula(paste("~",condition,"+venue_id"))))
    # clean the joint frequency table
    freq.joint = freq.joint[freq.joint$Freq>0,]
    freq.joint[,condition]=factor(freq.joint[,condition])
    
    N = sum(freq.joint$Freq)
    L = length(unique(data$venue_id))
    
    in.condition = do.call(rbind,lapply(split(freq.joint,freq.joint[,condition]),function(i){
        i$venue_id=factor(i$venue_id)
        # adjusted for small samples (Mueller, 1955)
        n = sum(i$Freq)
        H.v = with(i,sum( -1 * Freq/n * log2(Freq/n) + 1/2/n))
        # adjusted for small samples ("entropy estimates of small datasets")
#         H.v = sum(apply(i,1,function(j){
#             ni = as.integer(j["Freq"])
...
#         }))
        ...
        condition.value=i[1,condition]
        data.global.sub = data.global[as.character(data.global[,condition])==as.character(condition.value),]
        data.global.sub$venue_id = factor(data.global.sub$venue_id)
        prior.freq = as.data.frame(table(data.global.sub$venue_id))
        colnames(prior.freq) = c("venue_id","Freq")
        prior.freq$prob = prior.freq$Freq / sum(prior.freq$Freq)
        prior.freq = subset(prior.freq,as.character(venue_id) %in% as.character(i$venue_id))

        pi.min = get.pi.min.from.freq(i, prior.freq$prob) 
#         pi.min.v2 = get.pi.min.from.freq(i, 1/(l+1)) 
        data.frame("entropy"=H.v,
                   "pi.min"=pi.min,
                   "pi.max"=pi.max,
                   "record"=n,
    ...
    pi.min = with(in.condition, wt.mean(pi.min, record))
    
    list("mean"=data.frame("Condition"=condition.name,"Entropy"=H,
               "Pi.max"=pi.max,"Pi.min"=mean(in.condition$pi.min,na.rm=T)),
         "detail"=in.condition)
}
...
get.pi.min.from.freq = function(freq,prior,confidence=0.95){
    # pi.min should also consider the small sample correction
    # using Wilson score interval
    # extended from Wilson 1927
    if(sum(freq$Freq)>0){
...
entropies.etc.list = lapply(1:3,function(city.index){
    ...
    data.global$ST = paste(data.global$hour, data.global[,city.guide$spatial.attr[city.index]])
    
    counter.reset()
    entropies.etc=lapply(user.list,function(i){
        counter.print(2)
        
        rand = get.rand.entropy.etc(i)
        unc = get.unc.entropy.etc.v2(i,data.global)
        time = get.conditional.entropy.etc.v2(i,"hour","Hour",data.global)
        space = get.conditional.entropy.etc.v2(i,city.guide$spatial.attr[city.index],"Space",data.global)
        
        i$ST = paste(i$hour, i[,city.guide$spatial.attr[city.index]])
        st = get.conditional.entropy.etc.v2(i,"ST","ST",data.global)
...
#     time.space = data.frame(do.call(rbind,strsplit(as.character(df$Condition_ID),
#                                                    " ",fixed=TRUE)))
...
data=melt(entropies.etc.df[,c(1,3,4,6,9)],id.vars=c("Records","City","Condition"))
...
gga<-ggplot(data,aes(x=hour,y=pi) )+
    facet_wrap(~city)+
    geom_point(aes(shape=type))+
    geom_smooth(aes(group = type),se=F, method = "lm", formula = y ~ poly(x, 12),size=1) +
    xlab("Hour of Day\n(a)") +
...
ggb<-ggplot(data,aes(x=hour,y=record))+
    ...
    theme(strip.text=element_text(size=7),axis.text = element_text(size=6),
              axis.title = element_text(size=7),plot.title = element_text(size=8),
          plot.margin=unit(c(.05,.05,.05,.05),"npc"))
eqs <- ddply(data,.(city,type),function(i){
    model = lm(i,formula=pi~record) 
...
eqs$pos.y = rep(c(0.7,0.45),3)
ggc<-ggplot(data,aes(x=record,y=pi))+geom_point(size=1)+
    facet_grid(type~city,scales="free_y")+
    geom_smooth(method=lm,formula=y~x)+
    xlab("Check-ins\n(c)") +
    ylab(expression(Pi^min)) +ggtitle("Time Correlation")+
    scale_x_continuous(labels=percent)+
    geom_text(data=eqs,aes(x=0.005,y=pos.y,label=V1),inherit.aes=FALSE,
                  parse = TRUE,hjust=0,vjust=0.8,size=3)+
    theme(strip.text=element_text(size=7),axis.text = element_text(size=6),
              axis.title = element_text(size=7),plot.title = element_text(size=8),
          plot.margin=unit(c(.05,.05,.05,.05),"npc"))
grid.arrange(arrangeGrob(gga,ggb,nrow=2,heights=c(1.5,1)),ggc,
...
eqs2 <- ddply(data3,.(city,variable),function(i){
...
ggd<-ggplot(data3,aes(x=record,y=value))+facet_grid(city~variable)+
    geom_point(size=1)+
