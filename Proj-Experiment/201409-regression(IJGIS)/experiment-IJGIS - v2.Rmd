...
checkin.poly.list=list(checkin.poly.NY,checkin.poly.LA,checkin.poly.CH)
...
grid.arrange(
    point.plot(checkin.NY,alpha=0.3,size=1,shape=21,fill="white",
        ...
        annotate("text",x=-74.22,y=40.82,size=2.5,hjust=0,label="Area: 468.9 sq mi\nPopulation: 8,405,837(2013)\nCheck-ins: 579,786"),
    ...
    point.plot(checkin.LA,alpha=0.3,size=1,shape=21,fill="white",
        ...
        xlab("Longitude")+ylab("Latitude")+
        annotate("text",x=-118.62,y=33.82,size=2.5,hjust=0,label="Area:503 sq mi\nPopulation:3,884,307(2013)\nCheck-ins:138,211"),
    ...
    point.plot(checkin.CH,alpha=0.3,size=1,shape=21,fill="white",
        ...
        xlab("Longitude")+ylab("Latitude")+
        annotate("text",x=-87.92,y=41.7,size=2.5,hjust=0,label="Area:234.0 sq mi\nPopulation:2,695,598(2010)\nCheck-ins:183,837"),
...
```{r}
gg.personal.stat<-lapply(checkin.poly.list,function(checkin){
...
#     count.freq$Prob=count.freq$Freq.Freq/sum(count.freq$Freq.Freq)
    ...
    model = glm(data=count.freq,formula=Freq.Freq~log(Freq),
    ...
    gg<-ggplot(count.freq,aes(x=Freq,y=Freq.Freq))+
        geom_point(shape=21,fill="white",alpha=.5)+
        scale_x_log10()+
        geom_smooth(formula=y~x,method=glm,family=gaussian(link = 'log'))+
        xlab("Number of Check-ins [x]")+
        ylab("Number of Users\nHaving x Check-ins [y=f(x)]")+
        theme_grey(base_size = 10)
    list(gg,a=a,k=k)
})
model.summary.NY1=as.character(as.expression(substitute(italic(y) == e^a %.% italic(x)^k,
                         list(a = format(gg.personal.stat[[1]]$a, digits = 3), 
                              k = format(gg.personal.stat[[1]]$k, digits = 3)))))
model.summary.NY2 = "ANOVA: 
--------------------------------------------------------------------------------
            Df    Deviance    Resid.Df    Resid.Dev    Pr(>Chi)
NULL                                   356     177403499              
log(x)     1  175119543         355         2283955     < 2.2e-16 ***
--------------------------------------------------------------------------------
Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1"
model.summary.LA1=as.character(as.expression(substitute(italic(y) == e^a %.% italic(x)^k,
                         list(a = format(gg.personal.stat[[2]]$a, digits = 3), 
                              k = format(gg.personal.stat[[2]]$k, digits = 3)))))
model.summary.LA2 = "ANOVA: 
--------------------------------------------------------------------------------
            Df    Deviance    Resid.Df    Resid.Dev    Pr(>Chi)
NULL                                    181      51559695              
log(x)     1    51169528          180          390166    < 2.2e-16 ***
--------------------------------------------------------------------------------
Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1"
model.summary.CH1=as.character(as.expression(substitute(italic(y) == e^a %.% italic(x)^k,
                         list(a = format(gg.personal.stat[[3]]$a, digits = 3), 
                              k = format(gg.personal.stat[[3]]$k, digits = 3)))))
model.summary.CH2 = "ANOVA: 
--------------------------------------------------------------------------------
            Df    Deviance    Resid.Df    Resid.Dev    Pr(>Chi)
NULL                                    244      36737034              
log(x)     1   36624722           243          112311    < 2.2e-16 ***
--------------------------------------------------------------------------------
Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1"

tiff("plots/user.count.stat.tiff",width=12*ppi,height=3*ppi,res=ppi)
grid.arrange(gg.personal.stat[[1]][[1]]+ggtitle("New York City")+
                 geom_text(aes(x=5,y=10500,label=model.summary.NY1),parse = TRUE,hjust=0,size=3)+
                 geom_text(aes(x=5,y=6000,label=model.summary.NY2),hjust=0,size=2),
             gg.personal.stat[[2]][[1]]+ggtitle("Los Angeles City")+
                 geom_text(aes(x=5,y=6000,label=model.summary.LA1),parse = TRUE,hjust=0,size=3)+
                 geom_text(aes(x=5,y=3500,label=model.summary.LA2),hjust=0,size=2),
             gg.personal.stat[[3]][[1]]+ggtitle("Chicago City")+
                 geom_text(aes(x=5,y=5000,label=model.summary.CH1),parse = TRUE,hjust=0,size=3)+
                 geom_text(aes(x=5,y=3000,label=model.summary.CH2),hjust=0,size=2),
...
## plotting
tiff("plots/entropies.tiff",width=12*ppi,height=6*ppi,res=ppi)
